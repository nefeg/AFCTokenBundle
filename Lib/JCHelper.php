...
//
//		return JCJSON::factory( static::serializeToJSON($data, $serializeNull) );
//	}
     ...
     * @return string
     */
	static public function serialize($data, $format = 'json', bool $serializeNull = true, $groups = []) {

        $groups = count($groups) ? $groups : ['Default'];

		return SerializerBuilder::create()->build()->serialize(
			$data,
			$format,
	 ...
	 * @return null|string
	 */
    static function getTimeZoneName(Request $request) :?string{

	    $timezoneName = null;

	    if (strlen($request->server->get('HTTP_X_REQUESTTIMEZONE'))) {
            $timezoneName = timezone_name_from_abbr('', $request->server->get('HTTP_X_REQUESTTIMEZONE') * 60, false) ?: null;
        }
    ...
    /**
     * example -- ios: JellyChip/0.1 (com.jellychip; build:0.1; iOS 10.2.0) Alamofire/4.2.0
     *
     * @param Request $request
     * @return null|string
     */
    static function getBuildIOSAppVersion(Request $request) {

	    return preg_match('/(?<=JellyChip\/)([\d\.]+)(?=.+iOS)/i', $request->headers->get('user-agent', null, true), $version)
		    ? $version[0]
		    : null;
    ...
    /**
     * example -- JellyChip/v2.3 (com.jellychip; build:238; android v6.0; user:2470)
     *
     * @param Request $request
     * @return null|string
     */
    static function getBuildAndroidAppVersion(Request $request) {

	    return preg_match('/(?<=JellyChip\/v)([\d\.]+)(?=.+android)/i', $request->headers->get('user-agent', null, true), $version)
		    ? $version[0]
		    : null;
    }
